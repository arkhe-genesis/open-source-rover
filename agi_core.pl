...
detect_jailbreak(Text, _Pattern) :-
...
detect_injection(Text, _Pattern) :-
    ( string(Text) -> atom_string(Atom, Text) ; Atom = Text ),
    downcase_atom(Atom, Low),
...
positive_word(cannot). positive_word(no).
negative_word(bad). negative_word(terrible). negative_word(impossible). negative_word(never).
negative_word(false). negative_word(deny).
...
nwn_reservoir_compute(Input, _State, Output) :-
...
coordinate_substrates(_State, Coordinated) :-
    findall(S-Phase, (
        member(S, [163,164,168,172,184,202,207,208,211]),
        substratum_phase(S, Phase)
    ), Phases),
